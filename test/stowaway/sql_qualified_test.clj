...
(deftest apply-sort-to-a-query
  (is (= ["SELECT * FROM users ORDER BY users.last_name ASC, users.first_name DESC"]
         (-> (h/select :*)
             (h/from :users)
             (sql/apply-sort {:sort [:user/last-name [:user/first-name :desc]]})
             hsql/format))
  ...
  (is (= ["SELECT * FROM users"]
         (-> (h/select :*)
             ...
             (sql/apply-sort {})
             hsql/format))
      "No ORDER BY clauses is added of the :sort key is absent"))
...
(deftest apply-criteria-to-sql
  (is (= {:where [:= :first_name "John"]}
         (sql/apply-criteria {} #:user{:first-name "John"}))
      "A single-field equality can be added")
  (is (= {:where [:and
                  [:= :name "John"]
                  [:= :age 25]]}
         (sql/apply-criteria {} #:user{:name "John"
                                       :age 25}))
